...
class SimpleFileContent(file: SimpleFile, 
                        ...
                        otierNodeSize: Option[Int]=None) extends Logging {
  import SimpleFileContent._
...
object SimpleFileContent {
  ...
  private class Tail(val offset: Long,
                     ...
                     val data: DataBuffer,
                     val tkvl: TieredKeyValueList) {
    def size: Int = data.size
  }

  private object Tail {
    def apply(offset: Long, pointer: DataObjectPointer, revision: ObjectRevision, data: DataBuffer, tkvl: TieredKeyValueList): Tail = {
