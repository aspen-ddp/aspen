...
/** Used when procedure adding content to a transaction detects a logical flaw that causes the overall
  ...
  *
  * For example, while attempting to insert key/value pairs into a BTree hostState it could be discovered that one or more
  * of the keys is outside the valid key range owned by the hostState. In this case, the transaction cannot be allowed to
