...
class Registry(val client: AspenClient,
  ...
  /** Fails with KeyAlreadyExists if the key is taken. On a non-empty tree the
   *  requirement's abortAndThrow raises it directly, before this insert stages anything on
   *  the transaction.
   */
